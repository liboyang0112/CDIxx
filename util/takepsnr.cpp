...
Real* ssim_map(Real* img1, Real* img2, int row, int col, Real sigma){
  ...
  myCuDMalloc(Real, mu1mu2, row*col);
  myCuDMalloc(Real, mu1sq, row*col);
  myCuDMalloc(Real, mu2sq, row*col);
  ...
  //calculate sigmas first, use mu1 mu1mu2 mu1sq mu2sq as cache
  Real* img1sq = mu1sq, *img2sq = mu2sq, *img12 = mu1mu2;
  getMod2(img1sq, img1);
  getMod2(img2sq, img2);
  multiply(img12, img1, img2);
  applyGaussConv(img1sq, sigma1sq, kernel, sigma, 5);
  applyGaussConv(img2sq, sigma2sq, kernel, sigma, 5);
  applyGaussConv(img12, sigma12, kernel, sigma, 5);

  applyGaussConv(img1, mu1, kernel, sigma, 5);
  applyGaussConv(img2, mu1mu2, kernel, sigma, 5);
  getMod2(mu1sq, mu1);
  getMod2(mu2sq, mu1mu2);
  multiply(mu1mu2, mu1, mu1mu2);
  add(sigma1sq, mu1sq, -1);
  add(sigma2sq, mu2sq, -1);
  add(sigma12, mu1mu2, -1);
  ssimMap(mu1, mu1sq, mu2sq, mu1mu2, sigma1sq, sigma2sq, sigma12, C1, C2);
  return mu1;
}
