...
int main () {
  ...
  myCuDMalloc(complexFormat, img3, row*col);
  init_cuda_image();
  ...
  int maxn = 40;
  Real radius = 500;
  multiplyZernikeConj(img, img, radius, 0, 0);
  ...
  //add(img, img2);
  plt.init(row,col);
  plt.plotComplexColor(img, 0, 0.5, "output");
  ...
  zernike_compute(handle , img, row/2-0.5, col/2-0.5,radius);
  //loop(i, 1000){
  //  zernike_reconstruct(handle, img3, radius);
  //  add(img2, img2);
  //  add(img2, img3, -1);
  //  zernike_compute(handle , img2, row/2-0.5, col/2-0.5,radius);
  ...
  add(img, img2, -1);
  plt.plotComplexColor(img, 0, 1, "residual", 1, 0);
