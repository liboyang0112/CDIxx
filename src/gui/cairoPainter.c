...
CairoPainter *cairo_painter_new(const char *fname) {
  ...
  cp->isColor = 1;
  if (f.type == REALIDX)
    processFloat(cache, cp->d_image, cp->m, cp->isFrequency, cp->decay,
                 cp->islog, cp->isFlip, cp->isColor);
  if (f.type == COMPLEXIDX)
    processComplex(cache, cp->d_image, cp->m, cp->isFrequency, cp->decay,
                   cp->islog, cp->isFlip, cp->isColor);
  cp->buffer = gdk_pixbuf_new_from_data(cache, GDK_COLORSPACE_RGB, 0, 8, f.rows,
                                        f.cols, 3 * f.rows, 0, 0);
